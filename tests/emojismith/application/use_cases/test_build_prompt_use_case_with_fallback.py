...
from emojismith.domain.services.description_quality_analyzer import DescriptionQualityAnalyzer
...
async def test_build_prompt_with_poor_description_uses_fallback(
    ...
    # Build prompt
    result = await use_case.build_prompt(spec, enhance=True)
    ...
    assert any(word in prompt_arg.lower() for word in ["deployed", "payment", "system", "team"])
...
async def test_build_prompt_logs_fallback_usage(
    ...
    assert any("Using fallback prompt generation" in record.message for record in caplog.records)
...
async def test_configurable_quality_threshold(
    ...
    assert any(word in prompt_arg.lower() for word in ["completed", "onboarding", "user"])
