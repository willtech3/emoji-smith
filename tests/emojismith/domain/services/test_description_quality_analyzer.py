...
class TestDescriptionQualityAnalyzer:
    ...
    def test_analyze_vague_single_word_descriptions(self):
        ...
        vague_descriptions = ["emoji", "nice", "good", "bad", "thing", "stuff", "icon", "image"]
    ...
    def test_analyze_medium_quality_descriptions(self):
        ...
        for desc, min_score, max_score in test_cases:
            score, issues = analyzer.analyze_description(desc)
            assert min_score <= score <= max_score, f"'{desc}' score {score} not in range [{min_score}, {max_score}]"
    ...
    def test_generate_fallback_prompt_with_good_context(self):
        ...
        assert "payment" in fallback.lower() or "feature" in fallback.lower() or "team" in fallback.lower()
    ...
    def test_configurable_quality_threshold(self):
        ...
        assert analyzer.is_poor_quality("detailed robot with glowing blue eyes and metallic finish") is False
    ...
    def test_extract_concepts_from_context(self):
        ...
        assert any(concept in ["deployed", "authentication", "system", "production"] for concept in concepts)
