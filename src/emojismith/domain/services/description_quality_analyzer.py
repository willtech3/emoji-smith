...
class DescriptionQualityAnalyzer:
    ...
    VAGUE_TERMS: Set[str] = {
        'emoji', 'nice', 'good', 'bad', 'thing', 'stuff', 'icon', 
        'image', 'one', 'some', 'any', 'ok', 'okay'
    }
    
    MIN_MEANINGFUL_WORDS = 2
    
    VISUAL_TERMS: Set[str] = {
        'color', 'colorful', 'bright', 'dark', 'light', 'shape', 'size', 
        'style', 'look', 'appearance', 'red', 'blue', 'green', 'yellow',
        'orange', 'purple', 'pink', 'black', 'white', 'gray', 'grey',
        'small', 'big', 'large', 'tiny', 'huge', 'round', 'square',
        'shiny', 'matte', 'glossy', 'transparent', 'opaque'
    }
    
    EMOTION_TERMS: Set[str] = {
        'happy', 'sad', 'angry', 'excited', 'tired', 'sleepy', 'energetic',
        'calm', 'stressed', 'relaxed', 'worried', 'confident', 'nervous',
        'joyful', 'frustrated', 'content', 'anxious', 'proud', 'surprised'
    }
    
    ACTION_TERMS: Set[str] = {
        'running', 'jumping', 'dancing', 'sleeping', 'working', 'playing',
        'eating', 'drinking', 'flying', 'swimming', 'walking', 'sitting',
        'standing', 'laughing', 'crying', 'smiling', 'thinking', 'coding'
    }
    ...
    def __init__(self, quality_threshold: float = 0.5) -> None:
        ...
        Args:
            quality_threshold: Threshold below which descriptions are considered poor (0-1)
        """
    ...
    def analyze_description(self, description: str) -> Tuple[float, List[str]]:
        ...
        has_visual = any(
            word in words for word in self.VISUAL_TERMS
        )
    ...
    def extract_concepts(self, context: str) -> List[str]:
        ...
        stopwords = {
            ...
            'about', 'into', 'through', 'just', 'now', 'then', 'also'
        }
        ...
        for word in words:
            ...
            if (cleaned and 
                ...
                cleaned not in concepts):
                concepts.append(cleaned)
        
        # Prioritize action words and nouns
        priority_concepts = []
        for concept in concepts:
            if any(keyword in concept for keyword in [
                'deploy', 'ship', 'launch', 'release', 'feature', 'system',
                'payment', 'authentication', 'team', 'success', 'complete',
                'finish', 'build', 'create', 'update', 'fix', 'improve'
            ]):
                priority_concepts.append(concept)
        
        # Return priority concepts first, then others
        return priority_concepts[:3] + [c for c in concepts if c not in priority_concepts][:2]
    ...
    def generate_fallback_prompt(self, context: str, poor_description: str) -> str:
        ...
        desc_words = [
            w for w in poor_description.lower().split() 
            if w not in self.VAGUE_TERMS and len(w) > 2
        ]
